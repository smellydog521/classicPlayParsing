# calculate the probabilities of every weighted subgraph of 5 nodes
...
def main(argv):
	...
	convergence_file = open('convergence_weighted_5_node.txt','a')
	...
	for parents,dirnames,filenames in os.walk(argv[1]):		
		for fn in filenames:
			if 'weighted_5_node_subgraph.txt' in fn and 'edges' not in fn:
				...
				rf = open(argv[1]+'/'+fn,'r')
				f = open(argv[1]+'/'+fn[:fn.index('_')]+'_weighted_5_node_subgraph_original.txt','w+')
				con_file = open(argv[1]+'/'+fn[:str(fn).index('_')]+'_weighted_5_node_subgraph_convergence.txt','w+')
